...
import { needsWebSearch } from "@/lib/needWebSearch";
import { generateTitle } from "@/lib/generateTitle";
import { getRetriever } from "@/lib/langchain/retriever";
...
export async function POST(req: Request) {
  ...
  if (/^(hi|hello|hey|hii|hiii)$/i.test(finalQuestion.trim())) {
    const greeting = "Hi! 😊 How can I help you today?";
  // saving this atatic message reply to db as label of assistant
    await db.insert(messages).values({
      chatId,
      userId,
      role: "assistant",
...
`;
  ...
  const shouldSearch = await needsWebSearch(classifierInput);
  console.log("🤖 shouldSearch:", shouldSearch);
...
`,
  ...
  /* 📄 Vector Search (PDF RAG) */
 const retriever = await getRetriever(chatId);
 console.log(retriever , "retrieverrrrrrrrrrrrrr")
 
const docs = await retriever.invoke(finalQuestion);

console.log(docs , "docs")
const topChunks = docs.map((d) => d.pageContent).join("\n\n");
const hasPdfContext = topChunks.trim().length > 50;
